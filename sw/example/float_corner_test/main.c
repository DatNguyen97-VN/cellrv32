 ...
 * @brief Verification program for the CELLRV32 'Zfinx' extension (floating-point in x registers)
...
/**********************************************************************//**
 * Main function; test all available operations of the CELLRV32 'Zfinx' extensions using bit
...
// Disable compilation by default
  ...
  // intro
  //cellrv32_uart0_printf("INFO! Program has been compiled.\n");
  cellrv32_uart0_printf("<<< Zfinx extension corner test >>>\n");
#if (SILENT_MODE != 0)
...
// 0x7F00_0000
  ...
  for (i=0;i<254; i++) {
    ...
    if (cellrv32_cpu_csr_read(CSR_MCAUSE) == TRAP_CODE_I_ILLEGAL) {
      ...
      cellrv32_uart0_printf("Conversion of opa with %d exponent timed out\n",opa.binary_value>>23);
      err_cnt_total++;
    }
    cellrv32_cpu_csr_write(CSR_MCAUSE, 0);
...
// 0x0080_0000
  ...
  for (i=0;i<253; i++) {
    ...
    if (cellrv32_cpu_csr_read(CSR_MCAUSE) == TRAP_CODE_I_ILLEGAL) {
      cellrv32_uart0_printf("%c[1m[FAILED]%c[0m\n", 27, 27);
      cellrv32_uart0_printf("Addition using opa with %d exponent and opb with %d exponent timed out\n",opa.binary_value>>23,opb.binary_value>>23);
      err_cnt_total++;
    }
    cellrv32_cpu_csr_write(CSR_MCAUSE, 0);
  ...
  for (i=0;i<253; i++) {
    ...
    if (cellrv32_cpu_csr_read(CSR_MCAUSE) == TRAP_CODE_I_ILLEGAL) {
      cellrv32_uart0_printf("%c[1m[FAILED]%c[0m\n", 27, 27);
      cellrv32_uart0_printf("Subtraction using opa with %d exponent and opb with %d exponent timed out\n",opa.binary_value>>23,opb.binary_value>>23);
      err_cnt_total++;
    }
    cellrv32_cpu_csr_write(CSR_MCAUSE, 0);
    // increment opb exponent by 1
