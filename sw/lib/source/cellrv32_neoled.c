// #################################################################################################
// # << CELLRV32: cellrv32_neoled.c - Smart LED Interface (NEOLED) HW Driver >>                    #
// # ********************************************************************************************* #
 ...
 **************************************************************************/
int cellrv32_neoled_available(void) {

  if (CELLRV32_SYSINFO->SOC & (1 << SYSINFO_SOC_IO_NEOLED)) {
    return 1;
  }
  else {
    return 0;
...
/**********************************************************************//**
 * Enable and configure NEOLED controller. The NEOLED control register bits are listed in #CELLRV32_NEOLED_CTRL_enum.
 ...
 **************************************************************************/
void cellrv32_neoled_setup(uint32_t prsc, uint32_t t_total, uint32_t t_high_zero, uint32_t t_high_one, int irq_mode) {

  CELLRV32_NEOLED->CTRL = 0; // reset
 ...
 * @param[in] irq_mode Interrupt condition (1=IRQ if FIFO is empty, 1=IRQ if FIFO is less than half-full).
 **************************************************************************/
void cellrv32_neoled_setup_ws2812(int irq_mode) {
  ...
  uint32_t t_clock_x500ps = (2 * 1000 * 1000 * 1000) / CELLRV32_SYSINFO->CLK;
...
//cellrv32_uart0_printf("NEOLED.clk_prsc: %u\n", clk_prsc_fac); // DEBUG

    t_base = t_clock_x500ps * clk_prsc_fac;

//cellrv32_uart0_printf("NEOLED.t_base: %u x 0.5ns\n", t_base); // DEBUG
...
//cellrv32_uart0_printf("NEOLED.t_total:     %u\n", t_total); // DEBUG
//cellrv32_uart0_printf("NEOLED.t_high_zero: %u\n", t_high_zero); // DEBUG
//cellrv32_uart0_printf("NEOLED.t_high_one:  %u\n", t_high_one); // DEBUG
  ...
  cellrv32_neoled_setup(clk_prsc_sel, t_total, t_high_zero, t_high_one, irq_mode);
}
 ...
 **************************************************************************/
void cellrv32_neoled_set_mode(uint32_t mode) {

  uint32_t ctrl = CELLRV32_NEOLED->CTRL;
  ctrl &= ~(0b1 << NEOLED_CTRL_MODE); // clear current mode
  ctrl |= ((mode & 1) << NEOLED_CTRL_MODE); // set new mode
  CELLRV32_NEOLED->CTRL = ctrl;
}
 ...
 **************************************************************************/
void cellrv32_neoled_strobe_blocking(void) {

  while(1) { // wait for FIFO full flag to clear
    if ((CELLRV32_NEOLED->CTRL & (1 << NEOLED_CTRL_TX_FULL)) == 0) {
      break;
    }
  }

  cellrv32_neoled_strobe_nonblocking();
}
 ...
 **************************************************************************/
void cellrv32_neoled_strobe_nonblocking(void) {

  const uint32_t mask = 1 << NEOLED_CTRL_STROBE; // strobe bit
  uint32_t ctrl = CELLRV32_NEOLED->CTRL;

  CELLRV32_NEOLED->CTRL = ctrl | mask; // set strobe bit
  CELLRV32_NEOLED->DATA = 0; // send any data to trigger strobe command
  CELLRV32_NEOLED->CTRL = ctrl & (~mask); // clear strobe bit
}
 ...
 **************************************************************************/
void cellrv32_neoled_enable(void) {

  CELLRV32_NEOLED->CTRL |= ((uint32_t)(1 << NEOLED_CTRL_EN));
}
 ...
 **************************************************************************/
void cellrv32_neoled_disable(void) {
 ...
 **************************************************************************/
void cellrv32_neoled_write_blocking(uint32_t data) {
  ...
  cellrv32_neoled_write_nonblocking(data); // send new LED data
}
 ...
 **************************************************************************/
uint32_t cellrv32_neoled_get_buffer_size(void) {

  uint32_t tmp = CELLRV32_NEOLED->CTRL;
