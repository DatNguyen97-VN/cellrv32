// #################################################################################################
// # << CELLRV32 - SPI Bus Explorer Demo Program >>                                                #
// #################################################################################################
...
int main() {
  ...
  // info
  cellrv32_uart0_printf("This program allows to create SPI transfers by hand.\n"
  ...
  // Main menu
  for (;;) {
    cellrv32_uart0_printf("SPI_EXPLORER:> ");
...
void spi_cs(uint32_t type) {
  ...
  if (type) {
    cellrv32_uart0_printf("Chip-select line to ENABLE (set low) [0..7]: ");
    while (1) {
    ...
    cellrv32_spi_cs_en(channel);
  }
  else {
    cellrv32_uart0_printf("Disabling chip select lines.\n");
    cellrv32_spi_cs_dis();
  }
}
...
void spi_trans(void) {
  ...
  if (spi_configured == 0) {
    cellrv32_uart0_printf("SPI module not configured yet! Use 'setup' to configure SPI module.\n");
    return;
  }

  cellrv32_uart0_printf("Enter TX data (2 hex chars): 0x");
  cellrv32_uart0_scan(terminal_buffer, 2+1, 1);
  uint32_t tx_data = (uint32_t)hexstr_to_uint(terminal_buffer, strlen(terminal_buffer));

  uint32_t rx_data = cellrv32_spi_trans(tx_data);
...
void spi_setup(void) {
  ...
  while (1) {
    cellrv32_uart0_printf("Select SPI clock prescaler (0..7): ");
    cellrv32_uart0_scan(terminal_buffer, 2, 1);
  ...
  uint32_t clock = CELLRV32_SYSINFO->CLK / (2 * PRSC_LUT[spi_prsc] * (1 + clk_div));
  cellrv32_uart0_printf("\n+ New SPI clock speed = %u Hz\n", clock);

  // ---- SPI clock mode ----

  while (1) {
    cellrv32_uart0_printf("Select SPI clock mode (0..3): ");
    cellrv32_uart0_scan(terminal_buffer, 2, 1);
    tmp = (uint32_t)hexstr_to_uint(terminal_buffer, strlen(terminal_buffer));
    if (tmp > 4) {
      cellrv32_uart0_printf("\nInvalid selection!\n");
  ...
  cellrv32_spi_setup(spi_prsc, clk_div, clk_phase, clk_pol, 0);
