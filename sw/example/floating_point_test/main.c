// ##################################################################################################
// # << CELLRV32 - RISC-V Single-Precision Floating-Point 'Zfinx' Extension Verification Program >> #
// ##################################################################################################
 ...
 * @brief Verification program for the CELLRV32 'Zfinx' extension (floating-point in x registers) using pseudo-random data as input; compares results from hardware against pure-sw reference functions.
 **************************************************************************/

#include <cellrv32.h>
#include <float.h>
#include <math.h>
#include "cellrv32_zfinx_extension_intrinsics.h"
...
/**********************************************************************//**
 * Main function; test all available operations of the CELLRV32 'Zfinx' extensions using bit floating-point hardware intrinsics and software-only reference functions (emulation).
 *
...
// ----------------------------------------------------------------------------
  ...
  cellrv32_uart0_printf("FCVT.S.W: ");
  time_sw = 0;
  time_hw = 0;
  err_cnt = 0;
  for (i=0; i<num_runs; i++) {
    ...
    time_start = cellrv32_cpu_csr_read(CSR_CYCLE);
    {
      res_hw.float_value = riscv_intrinsic_fcvt_sw((int32_t)opa.binary_value);
    }
    time_hw += cellrv32_cpu_csr_read(CSR_CYCLE) - time_start;
    ...
    time_sw += cellrv32_cpu_csr_read(CSR_CYCLE) - time_start;
  ...
  if (err_cnt == 0) {
    cellrv32_uart0_printf("cycles[SW] = %u vs. cycles[HW] = %u\n", time_sw/num_runs, time_hw/num_runs);
  }
  else {
    cellrv32_uart0_printf("%c[1m[TEST FAILED!]%c[0m\n", 27, 27);
    err_cnt_total++;
  }


  // float to signed integer
  cellrv32_uart0_printf("FCVT.W.S: ");
  time_sw = 0;
  time_hw = 0;
  err_cnt = 0;
  for (i=0; i<num_runs; i++) {
  ...
  // Sign-injection
  cellrv32_uart0_printf("FSGNJX.S: ");
  time_sw = 0;
  time_hw = 0;
  err_cnt = 0;
  for (i=0; i<num_runs; i++) {
