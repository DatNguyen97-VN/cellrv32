...
/* Define : TIMER_RES_DIVIDER
   ...
   increase this value.
        */
#define GETMYTIME(_t)              (*_t = (CORETIMETYPE)cellrv32_cpu_get_cycle())
...
start_time(void)
{
    ...
    cellrv32_cpu_csr_write(CSR_MCOUNTINHIBIT, 0); // start all counters
}
/* Function : stop_time
...
stop_time(void)
{
    cellrv32_cpu_csr_write(CSR_MCOUNTINHIBIT, -1); // stop all counters
...
{
    /* CELLRV32-specific */
    secs_ret retval = (secs_ret)(((CORE_TICKS)ticks) / ((CORE_TICKS)CELLRV32_SYSINFO->CLK));
    return retval;
}
...
#endif
{
  ...
  cellrv32_rte_setup(); // capture all exceptions and give debug information, no HW flow control
...
// Disable coremark compilation by default
  ...
  cellrv32_uart0_printf("ERROR! CoreMark has not been compiled. Use >>make USER_FLAGS+=-DRUN_COREMARK clean_all exe<< to compile it.\n");
  ...
  cellrv32_uart0_printf("CELLRV32: Executing coremark (%u iterations). This may take some time...\n\n", (uint32_t)ITERATIONS);

  // clear cycle counter
  cellrv32_cpu_csr_write(CSR_MCOUNTEREN, -1); // enable access to all counters
...
portable_fini(core_portable *p)
{
  ...
  exe_instructions.uint64 = cellrv32_cpu_get_instret();

  cellrv32_uart0_printf("\nCELLRV32: All reported numbers only show the integer part.\n\n");

  cellrv32_uart0_printf("CELLRV32: HPM results (low words only)\n");
  if (num_hpm_cnts_global == 0) {cellrv32_uart0_printf("no HPMs available\n"); }
  if (num_hpm_cnts_global > 0)  {cellrv32_uart0_printf(" > Retired compr. instructions:  %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER3)); }
  if (num_hpm_cnts_global > 1)  {cellrv32_uart0_printf(" > Instr.-fetch wait cycles:     %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER4)); }
  if (num_hpm_cnts_global > 2)  {cellrv32_uart0_printf(" > Instr.-issue wait cycles:     %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER5)); }
  if (num_hpm_cnts_global > 3)  {cellrv32_uart0_printf(" > Multi-cycle ALU wait cycles:  %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER6)); }
  if (num_hpm_cnts_global > 4)  {cellrv32_uart0_printf(" > Load operations:              %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER7)); }
  if (num_hpm_cnts_global > 5)  {cellrv32_uart0_printf(" > Store operations:             %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER8)); }
  if (num_hpm_cnts_global > 6)  {cellrv32_uart0_printf(" > Load/store wait cycles:       %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER9)); }
  if (num_hpm_cnts_global > 7)  {cellrv32_uart0_printf(" > Unconditional jumps:          %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER10)); }
  if (num_hpm_cnts_global > 8)  {cellrv32_uart0_printf(" > Conditional branches (all):   %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER11)); }
  if (num_hpm_cnts_global > 9)  {cellrv32_uart0_printf(" > Conditional branches (taken): %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER12)); }
  if (num_hpm_cnts_global > 10) {cellrv32_uart0_printf(" > Entered traps:                %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER13)); }
  if (num_hpm_cnts_global > 11) {cellrv32_uart0_printf(" > Illegal operations:           %u\n", (uint32_t)cellrv32_cpu_csr_read(CSR_MHPMCOUNTER14)); }
  cellrv32_uart0_printf("\n");

  cellrv32_uart0_printf("CELLRV32: Executed instructions:       0x%x%x\n", (uint32_t)exe_instructions.uint32[1], (uint32_t)exe_instructions.uint32[0]);
  cellrv32_uart0_printf("CELLRV32: CoreMark core clock cycles:  0x%x%x\n", (uint32_t)exe_time.uint32[1], (uint32_t)exe_time.uint32[0]);

  uint64_t average_cpi_int = exe_time.uint64 / exe_instructions.uint64;
  cellrv32_uart0_printf("CELLRV32: Average CPI (integer  only): %u cycles/instruction\n", (uint32_t)average_cpi_int);
