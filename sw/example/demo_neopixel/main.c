// #################################################################################################
// # << CELLRV32 - Smart LED (NeoPixel/WS2812) Hardware Interface (NEOLED) Demo Program >>         #
// #################################################################################################
...
int main() {
  ...
  if (cellrv32_neoled_available() == 0) {
    cellrv32_uart0_printf("Error! No NEOLED unit synthesized!\n");
    return 1;
  }


  // illustrate setup
  cellrv32_uart0_printf("<<< CELLRV32 NeoPixel (WS2812) Controller (NEOLED) Demo Program >>>\n"
                       "(TM) 'NeoPixel' is a trademark of Adafruit Industries.\n\n"
                       "This demo uses the following LED setup:\n"
                       "CELLRV32.neoled_o -> %u RGB-LEDs (24-bit)\n\n", (uint32_t)NUM_LEDS_24BIT);


  // use the "cellrv32_neoled_setup_ws2812()" setup function here instead the raw "cellrv32_neoled_setup()"
  // cellrv32_neoled_setup_ws2812() will configure all timing parameters according to the WS2812 specs. for the current processor clock speed
  cellrv32_neoled_setup_ws2812(0); // interrupt configuration = fire IRQ if TX FIFO is empty (not used here)
  cellrv32_neoled_set_mode(0); // mode = 0 = 24-bit


  // check NEOLED configuration
  cellrv32_uart0_printf("Checking NEOLED configuration:\n"
                       " Hardware FIFO size: %u entries\n"
                       " Control register:   0x%x\n\n", cellrv32_neoled_get_buffer_size(), CELLRV32_NEOLED->CTRL);


  // clear all LEDs
  cellrv32_uart0_printf("Clearing all LEDs...\n");
  int i;
  for (i=0; i<NUM_LEDS_24BIT; i++) {
    cellrv32_neoled_write_blocking(0);
  }
  cellrv32_cpu_delay_ms(500);
  ...
  cellrv32_uart0_printf("Starting animation...\n");
