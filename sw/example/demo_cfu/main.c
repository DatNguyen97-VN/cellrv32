// #################################################################################################
// # << CELLRV32 - CFU: Custom Instructions Example Program >>                                     #
// #################################################################################################
 ...
 * 'rtl/core/cellrv32_cpu_cp_cfu.vhd'.
 **************************************************************************/
#include <cellrv32.h>
...
int main() {
  ...
  if (cellrv32_cpu_cfu_available() == 0) {
    cellrv32_uart0_printf("ERROR! CFU ('Zxcfu' ISA extensions) not implemented!\n");
    return 1;
  }


  // intro
  cellrv32_uart0_printf("\n<<< CELLRV32 Custom Functions Unit (CFU) - Custom Instructions Example Program >>>\n\n");

  cellrv32_uart0_printf("[NOTE] This program assumes the _default_ CFU hardware module, which\n"
...
/*
  ...
  There are 4 "prototype primitives" for the CFU instructions (define in sw/lib/include/cellrv32_cfu.h):

  > cellrv32_cfu_r3_instr(funct7, funct3, rs1, rs2) - for r3-type instructions (custom-0 opcode)
  > cellrv32_cfu_r4_instr(funct3, rs1, rs2, rs3)    - for r4-type instructions (custom-1 opcode)
  > cellrv32_cfu_r5_instr_a(rs1, rs2, rs3, rs4)     - for r5-type instruction A (custom-2 opcode)
  > cellrv32_cfu_r5_instr_b(rs1, rs2, rs3, rs4)     - for r5-type instruction B (custom-3 opcode)
  ...
  Each "cellrv32_cfu_r*" function returns a 32-bit data word of type uint32_t that represents
...
*/
  ...
  for (i=0; i<TESTCASES; i++) {
    rs1 = xorshift32();
    cellrv32_uart0_printf("%u: cellrv32_cfu_r3_instr( funct7=0b1111111, funct3=0b000, [rs1]=0x%x, [rs2]=0x%x ) = ", i, rs1, 0);
    // here we are setting the funct7 bit-field to all-one; however, this is not used at all by the default CFU hardware module.
    cellrv32_uart0_printf("0x%x\n", cellrv32_cfu_r3_instr(0b1111111, 0b000, rs1, 0));
  }
  ...
  for (i=0; i<TESTCASES; i++) {
    rs1 = xorshift32();
    rs2 = xorshift32();
    cellrv32_uart0_printf("%u: cellrv32_cfu_r3_instr( funct7=0b0000000, funct3=0b001, [rs1]=0x%x, [rs2]=0x%x ) = ", i, rs1, rs2);
    cellrv32_uart0_printf("0x%x\n", cellrv32_cfu_r3_instr(0b0000000, 0b001, rs1, rs2));
  }
...
#define madd_lo(a, b, c) cellrv32_cfu_r4_instr(0b000, a, b, c)
#define madd_hi(a, b, c) cellrv32_cfu_r4_instr(0b001, a, b, c)
  ...
  for (i=0; i<TESTCASES; i++) {
    ...
    rs2 = xorshift32();
    rs3 = xorshift32();
    cellrv32_uart0_printf("%u: cellrv32_cfu_r4_instr( funct3=0b000, [rs1]=0x%x, [rs2]=0x%x, [rs3]=0x%x ) = ", i, rs1, rs2, rs3);
    cellrv32_uart0_printf("0x%x\n", madd_lo(rs1, rs2, rs3));
  }
  ...
  for (i=0; i<TESTCASES; i++) {
    ...
    rs2 = xorshift32();
    rs3 = xorshift32();
    cellrv32_uart0_printf("%u: cellrv32_cfu_r4_instr( funct3=0b001, [rs1]=0x%x, [rs2]=0x%x, [rs3]=0x%x ) = ", i, rs1, rs2, rs3);
    cellrv32_uart0_printf("0x%x\n", madd_hi(rs1, rs2, rs3));
  }
  ...
  for (i=0; i<TESTCASES; i++) {
    ...
    rs3 = xorshift32();
    rs4 = xorshift32();
    cellrv32_uart0_printf("%u: cellrv32_cfu_r5_instr_a( [rs1]=0x%x, [rs2]=0x%x, [rs3]=0x%x, [rs3]=0x%x ) = ", i, rs1, rs2, rs3, rs4);
    cellrv32_uart0_printf("0x%x\n", cellrv32_cfu_r5_instr_a(rs1, rs2, rs3, rs4));
  }
  ...
  for (i=0; i<TESTCASES; i++) {
    ...
    rs3 = xorshift32();
    rs4 = xorshift32();
    cellrv32_uart0_printf("%u: cellrv32_cfu_r5_instr_b( [rs1]=0x%x, [rs2]=0x%x, [rs3]=0x%x, [rs3]=0x%x ) = ", i, rs1, rs2, rs3, rs4);
    cellrv32_uart0_printf("0x%x\n", cellrv32_cfu_r5_instr_b(rs1, rs2, rs3, rs4));
  }
  ...
  for (i=0; i<TESTCASES; i++) {
    ...
    cellrv32_uart0_printf("%u: cellrv32_cfu_r3_instr( funct7=0b0000000, funct3=0b111, [rs1]=0x%x, [rs2]=0x%x ) = ", i, rs1, rs2);
    cellrv32_uart0_printf("0x%x\n", cellrv32_cfu_r3_instr(0b0000000, 0b111, rs1, rs2));
  }
