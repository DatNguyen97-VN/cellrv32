...
if [ $# -ne 2 ]
then
  echo "Upload image via serial port (UART) to the CELLRV32 bootloader."
  echo "Reset processor before starting the upload."
  echo "Usage:   [sudo] sh uart_upload.sh <port> <CELLRV32 executable>"
  echo "Example: sh uart_upload.sh /dev/ttyS6 cellrv32_exe.bin"
  exit
fi
...
# check response
if ! grep -Fq "Awaiting cellrv32_exe.bin" uart_upload.response.dat;
then
