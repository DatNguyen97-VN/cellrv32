...
int OWSearch()
{
   ...
   {
      // 1-Wire reset
      if (cellrv32_onewire_reset_blocking())
      {
         // reset the search
         LastDiscrepancy = 0;
         LastDeviceFlag = FALSE;
      ...
      cellrv32_onewire_write_byte_blocking(0xF0);  

      // loop to do the search
      do
      {
         // read a bit and its complement
         id_bit = cellrv32_onewire_read_bit_blocking();
         cmp_id_bit = cellrv32_onewire_read_bit_blocking();
         ...
         else
         {
            ...
            cellrv32_onewire_write_bit_blocking(search_direction);
